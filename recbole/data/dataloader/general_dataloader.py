...
from recbole.utils import InputType, ModelType, FeatureType, FeatureSource
...
class FastSampleEvalDataLoader(AbstractDataLoader):
    def __init__(self, config, dataset, sampler, shuffle=False):
        user_num = dataset.user_num
        dataset.sort(by=dataset.uid_field, ascending=True)
        self.uid_field = dataset.uid_field
    ...
    def _next_batch_data(self):
        if not self.is_sequential:
            ...
            # neg sample
            item_id_with_negs = list()
            negs = self._neg_sampling(interaction).split(self.neg_sample_num)
            item_ids = interaction[self.iid_field].tolist()
            for i in range(len(item_ids)):
                negs_u = [item_ids[i]]
                negs_u.extend(negs[i].tolist())
                # print(negs_u)
                item_id_with_negs.append(negs_u)
            item_id_with_negs = torch.LongTensor(item_id_with_negs)
            interaction.update(Interaction({"item_id_with_negs": item_id_with_negs}))
            self.pr += self.step
            return interaction, None, positive_u, positive_i
