...
def init_logger(config, enable_fh, logfilename=None):
    """
    ...
    ensure_dir(dir_name)
    model_name = os.path.join(dir_name, config['model'], config['dataset'])
    ensure_dir(model_name)
    if logfilename is None:
        logfilename = '{}-{}.log'.format(config['dataset'], get_local_time())
    logfilepath = os.path.join(model_name, logfilename)
